...
function transformTmdbJson(tmdbData) {
  ...
  return tmdbData.results.map(movie => ({
    ...
    tags: movie.genre_ids.map(id => (genreMap[id] || "unknown").toLowerCase())
  }));
}
...
fs.writeFileSync('transformedMovies.json', JSON.stringify(transformTmdbJson(tmdbData), null, 2), 'utf8');